...
s_i = s_i.replace("**", "german")

ptf = r"D:\gthb\dwo\OEBPS\Text\*.xhtml"
ptf = r"d:\pd\*.md"
pto = r"D:\gthb\dwo\de\OEBPS\Text\\"
pto = r"d:\pd\\"
...
for f_name in f_list:
    ...
    if bgn <= str(Path(os.path.basename(f_name)).with_suffix("")) <= end:
        ...
        for i in range(0, lns, s):
            ...
            if not rs.endswith("\n"):
                rs += "\n"
            rs = rs.removesuffix("</html>\n")
            rs = rs.removesuffix("</html>")
            rs = rs.removesuffix("</body>\n")
            rs = rs.removesuffix("</body>")
